...
def render_humanize_start(
    *,
    ...
    if target_repo is not None:
        start_section = f"""## Target Repository Staging

This workspace is a staging and review workspace for a real code-changing task. Do **not** launch Claude Code from `{workspace}`.

Copy `docs/plan.md` into a normal tracked path under `{target_repo}`, commit it, and run `scripts/start_humanize_task.py` with both `--target-repo` and `--target-plan`. The start wrapper validates target-repository hygiene and generates `launch_humanize.sh`, which rechecks the target and launches Claude Code from the correct repository root.

Never place a tracked plan under `.humanize/`. Keep `.humanize/rlcr/` local-only, and never use `git add -f .humanize`.
"""
    else:
        start_section = f"""## Start Command
...
Workspace-only mode does not prove that this directory is a git repository and does not provide target-repository session-root guarantees.
"""
...
This file does not prove the Humanize Claude Code plugin is installed. A local `humanize/` checkout can help instruction discovery, but Claude Code needs the plugin commands registered before `/humanize:*` commands are available.
...
def prepare(args: argparse.Namespace) -> int:
    ...
    metadata = {
        ...
        "commands_run": commands_run,
        "next_command": None if target_repo is not None else "/humanize:start-rlcr-loop docs/plan.md",
    }
    ...
    if target_repo is not None:
        print("Next: copy and commit docs/plan.md in the target repository, then run start_humanize_task.py with --target-plan.")
    else:
