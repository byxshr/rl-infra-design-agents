...
|---|---|---|---|
| Core checks | done | `conda run -n rl-infra-design-agents make check` passed with content ledger validation, golden path snapshot tests, and 70 tests. | Keep green after every workflow or content change. |
...
| P1 rollout/backend/debug/async evidence | done | Standalone `RLInfraWiki` commit `b34816b` is pinned by the main repo submodule and includes rollout backend selection, training/rollout mismatch debugging, async agentic RL, and Ray orchestration evidence. | Keep source-reported backend/debug/async claims distinct from local runtime verification. |
...
| IMP-005 | P1 Wiki evidence | P1 | in-progress | Promote high-impact P1 Wiki tracks to `code-evidenced`. | Rollout backend selection, training/rollout mismatch debugging, async agentic RL, and Ray orchestration are complete through standalone `RLInfraWiki` commit `b34816b`; next promote training backend comparison entries from `docs/rlinfrawiki-content-status.md`. | Main repo `make check` passed with content ledger validation, golden path snapshots, and 70 tests; all four demos passed review gate; async agentic/Ray query and `compare_frameworks.py areal roll --capability async-rollout` passed. Runtime GPU/NCCL/multi-node/performance claims remain unverified. | 2026-06-15 |
...
3. Promote the next P1 Wiki tracks from `IMP-005` in standalone `RLInfraWiki/`, starting with training backend comparison, then update the main repo gitlink.
