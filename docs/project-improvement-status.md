...
|---|---|---|---|
| Core checks | done | `conda run -n rl-infra-design-agents make check` passed with 49 tests. | Keep green after every workflow or content change. |
...
| P1 rollout backend evidence | done | Standalone `RLInfraWiki` commit `bda61b7` is pinned by the main repo submodule and includes rollout backend selection code-evidenced content plus the dedicated renderer scaffold. | Keep source-reported backend claims distinct from local runtime verification. |
...
| CI | done | `.github/workflows/validate.yml` checks out submodules, installs dev dependencies, runs `make check`, renders/reviews P0, slime GRPO/RLVR data-contract, and rollout backend selection paths. | Watch the first remote Actions run after push and keep it green. |
...
| IMP-004 | Task contract library | P1 | in-progress | Cover common RL infra/algorithm design scenarios beyond weight sync. | Added rollout backend selection as the third golden path after P0 weight sync and slime GRPO/RLVR data-contract; next candidates are training/rollout mismatch debugging or async agentic RL. | `conda run -n rl-infra-design-agents make demo-rollout-backend-selection` passed and rendered a source-backed context bundle with four required packs. | 2026-06-13 |
| IMP-005 | P1 Wiki evidence | P1 | in-progress | Promote high-impact P1 Wiki tracks to `code-evidenced`. | Rollout backend selection content and renderer scaffold are complete in standalone `RLInfraWiki` commit `bda61b7`; next promote async agentic RL, Ray orchestration, and training backend comparison entries from `docs/rlinfrawiki-content-status.md`. | Rollout backend selection passed standalone `validate.py`, `generate_indices.py --check`, focused renderer/context tests, rendered workspace review gate, and `conda run -n rl-infra-design-agents pytest -q` with 49 tests. | 2026-06-13 |
