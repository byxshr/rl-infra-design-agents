...
| P1 rollout backend evidence | done | Standalone `RLInfraWiki` commit `07794e0` is pinned by the main repo submodule and promotes rollout backend selection to code-evidenced/source-reading content with vLLM/SGLang SourcePack refs. | Use this as the content base before adding the rollout backend selection golden path. |
...
| IMP-005 | P1 Wiki evidence | P1 | in-progress | Promote high-impact P1 Wiki tracks to `code-evidenced`. | Rollout backend selection is complete in standalone `RLInfraWiki` commit `07794e0`; next promote async agentic RL, Ray orchestration, and training backend comparison entries from `docs/rlinfrawiki-content-status.md`. | Rollout backend selection passed standalone `validate.py`, `generate_indices.py --check`, `repo_status.py`, two rollout backend queries, context bundle compose/validate, `verify_source_refs.py`, and `conda run -n rl-infra-design-agents pytest -q` with 48 tests. | 2026-06-13 |
...
2. Turn `rollout-backend-selection.yaml` into the third golden path using the newly code-evidenced RLInfraWiki content.
