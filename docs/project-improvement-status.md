...
|---|---|---|---|
| Core checks | done | `conda run -n rl-infra-design-agents make check` passed with content ledger validation, golden path snapshot tests, and 65 tests. | Keep green after every workflow or content change. |
...
| P1 rollout/backend/debug evidence | done | Standalone `RLInfraWiki` commit `b5eea9b` is pinned by the main repo submodule and includes rollout backend selection plus training/rollout mismatch debugging scaffolds. | Keep source-reported backend/debug claims distinct from local runtime verification. |
...
| IMP-005 | P1 Wiki evidence | P1 | in-progress | Promote high-impact P1 Wiki tracks to `code-evidenced`. | Rollout backend selection and training/rollout mismatch debugging content/scaffolds are complete in standalone `RLInfraWiki` commit `b5eea9b`; next promote async agentic RL, Ray orchestration, and training backend comparison entries from `docs/rlinfrawiki-content-status.md`. | Main repo `make check` passed with content ledger validation, golden path snapshots, and 65 tests; rollout/backend/debug demos passed review gate. Runtime GPU/NCCL/multi-node/performance claims remain unverified. | 2026-06-14 |
...
3. Promote the next P1 Wiki tracks from `IMP-005` in standalone `RLInfraWiki/`, starting with async agentic RL, Ray orchestration, or training backend comparison, then update the main repo gitlink.
