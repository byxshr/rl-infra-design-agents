...
| `conda run -n rl-infra-design-agents make check` | Passed: generated query indices current, RLInfraWiki validation passed, strict SourcePack hash validation passed, content ledger validation passed, policy-only `humanize-gate-invariants-v1` validation warned that no runtime root was supplied, golden path snapshot tests passed, `161 passed`. |
| Humanize-focused tests in the five root test modules | Passed: `77 tests`; includes target hygiene, bridge v1/v2 compatibility, operator schema v3, required clean `byxshr/humanize` fork provenance, missing/wrong/dirty fork rejection, runtime contract fingerprints, shared guidance-corpus compatibility, policy-only warning behavior, launcher `--plugin-dir` quoting, strict verdict import, legacy warning behavior, and no-partial-write rejection of gate-conflicting review output. |
...
| `conda run -n rl-infra-design-agents python -m pytest -q` via the main repo plus pinned `RLInfraWiki` dependency | Passed through `make check`: `161 passed`. Bare base `pytest` previously exited 139 in this environment, so conda env is the validated test runner. |
...
- `start-humanize-task` validates the selected Humanize runtime against `humanize-gate-invariants-v1`. Real target mode requires a clean local checkout with a remote for `byxshr/humanize`, defaults `HUMANIZE_PLUGIN_ROOT` to `../humanize`, and never falls back to marketplace discovery. Target runtime failures return `2`; hygiene failures return `3`. The validated fork is passed to Claude through `--plugin-dir`, and runtime provenance includes its Git HEAD/remote plus a deterministic SHA256 contract fingerprint so identical declared versions do not imply identical compatibility. Workspace-only mode retains installed-plugin discovery for design-task compatibility.
...
5. Keep the project-maintained `byxshr/humanize` checkout clean and use it for every real target task through the generated `--plugin-dir` launcher; do not substitute the marketplace runtime.
